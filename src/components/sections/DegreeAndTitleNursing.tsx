// components/DegreeAndTitleNursing.tsx
...
const CertificateIcon = ({ className }: { className?: string }) => (
    <svg className={className} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <path d="M12 12C9.23858 12 7 9.76142 7 7C7 4.23858 9.23858 2 12 2" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
        <path d="M12 22C14.7614 22 17 19.7614 17 17C17 14.2386 14.7614 12 12 12" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
    </svg>
);
...
const certifications = [
  {
    type: 'Grado',
    name: 'Bachiller Técnico de Enfermería Técnica',
    imageUrl: '/img/mi-imagen-de-grado.png', 
  },
  {
    type: 'Título',
    name: 'Profesional Técnico en Enfermería Técnica',
    imageUrl: '/img/titulo_profesional_enfer.jpg',
  },
];
  ...
  return (
    <section className="w-full bg-white py-16 sm:py-24">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
            ...
            return (
              <div key={index}>
                ...
                <button
                  ...
                  className={`w-full p-4 flex items-center justify-center gap-4 rounded-full border-2 transition-all duration-300 ${
                    isActive
                      ? 'bg-[#0A2540] text-white border-transparent shadow-lg'
                      : 'bg-white text-[#0A2540] border-slate-300 hover:border-[#0A2540]'
                  }`}
                >
                  <CertificateIcon className="w-6 h-6" />
                  <span className="font-semibold text-left">{cert.name}</span>
                </button>
              </div>
            );
          })}
        ...
        {/* --- MODIFICACIÓN: Se añadió max-w-4xl y mx-auto para limitar el tamaño --- */}
        <div className="mt-12 relative max-w-2xl mx-auto">
          {certifications.map((cert, index) => (
            ...
            >
              <Image
                src={cert.imageUrl}
                alt={`Imagen del certificado: ${cert.name}`}
                width={1000}
                height={707}
                layout="responsive"
                objectFit="contain"
